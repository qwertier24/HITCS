...
QEMUOPTS += -net user -net nic,model=e1000 -redir tcp:$(PORT7)::7 \
	-redir tcp:$(PORT80)::80 -redir tcp:$(PORT8080)::8080 \
	-redir udp:$(PORT7)::7 -net dump,file=qemu.pcap
QEMUOPTS += $(QEMUEXTRA)
...
git-handin: handin-check
	@if test -n "`git config remote.handin.url`"; then \
		...
		if ! git push -f handin HEAD; then \
			...
			echo "and visit https://tc.gtisc.gatech.edu/cs3210/2016/submit/"; \
...
WEBSUB = https://tc.gtisc.gatech.edu/cs3210/2016/submit/handin.py/

handin: tarball-pref myapi.key
	@SUF=$(LAB); \
	./tools/submit -f lab$$SUF-handin.tar.gz;
