...
boot_alloc(uint32_t n)
{
  ...
  if (PADDR(nextfree) > npages * PGSIZE)
    panic("bootalloc: Out of memory %lx %lx", result, npages * PGSIZE);
  return result;
}
...
{
  // LAB 3: Your code here.
  uintptr_t i = (uintptr_t)ROUNDUP((char*)va, PGSIZE);
  uintptr_t end = (uintptr_t)ROUNDUP((char*)va + len, PGSIZE);
  pte_t *pt;

  // first check the page va in
  pt = pgdir_walk(env->env_pgdir, (const void*)va, 0);
  ...
  // then check the page va + len in
  for (; i < end; i += PGSIZE) {
    pt = pgdir_walk(env->env_pgdir, (const void*)i, 0);
    if (!pt || i >= ULIM || !(PGOFF(*pt) & perm)) {
